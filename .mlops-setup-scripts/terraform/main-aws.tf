...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name           = "my-mlops-project-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "my-mlops-project-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for my-mlops-project"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "my-mlops-project-terraform-cicd-user"
}
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "my-mlops-project-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
